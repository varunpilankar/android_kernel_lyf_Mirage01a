...
//	char temp[255];
	...
	if (gpio_is_valid(gpio)) {
		...
		printk(INFODEBUG"reset gpio unvalid!!!\n");
	...
	*/
	...
	dev_info(&client->dev, INFODEBUG"mxt reset %ld, irq flags 0x%lx\n",
					...
					const struct i2c_device_id *id)
{
	...
	if (!data->pdata && client->dev.of_node)
		data->pdata = mxt_parse_dt(client); // parse devTree ---leibo
#endif
...
dev_info(&client->dev, INFODEBUG"%s: start irq thread!\n", 
		__func__);

#if defined(CONFIG_MXT_IRQ_WORKQUEUE) // ??? -- leibo
	...
	dev_info(&client->dev, INFODEBUG"%s: power on & reset!\n", 
			__func__);
	mxt_probe_regulators(data); //power on & power sequence(reset & power on) ---leibo
#if defined(CONFIG_MXT_EXTERNAL_TRIGGER_IRQ_WORKQUEUE) || defined(CONFIG_MXT_REPORT_VIRTUAL_KEY_SLOT_NUM)
	mxt_g_data = data;
#endif
	error = mxt_initialize(data); //read chip info, read bootloader info --leibo
	if (error)
		goto err_free_irq;
...
#if defined(CONFIG_FB_PM)
	data->fb_notif.notifier_call = fb_notifier_callback;// suspend & resume notifier chain in FB module --leibo
