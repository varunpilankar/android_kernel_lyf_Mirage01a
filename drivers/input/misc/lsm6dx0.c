...
struct lsm6dx0_status {
	...
	struct sensors_classdev gyr_cdev;
...
static struct sensors_classdev sensors_accel_cdev = {
	.name = "lsm6ds0_accelerometer",
	.vendor = "st",
	.version = 1,
...
static int lsm6d_acc_cdev_set_enable(struct sensors_classdev *sensors_cdev,
							unsigned int enable)
{
	struct lsm6dx0_status *stat =
	...
	dev_info(&stat->client->dev, "%s, enable %d\n", __FUNCTION__, enable);

	return 0;
}

static int lsm6d_acc_cdev_set_poll_delay(
		struct sensors_classdev *sensors_cdev, unsigned int msecs)
{
	struct lsm6dx0_status *stat =
		container_of(sensors_cdev, struct lsm6dx0_status, acc_cdev);

	msecs -= CTS_OPTIMISE_MS;
...
static int lsm6d_acc_cdev_calibrate(struct sensors_classdev *sensors_cdev,
		int axis, int apply_now)
{
	...
	ret = lsm6dx0_acc_get_data(stat, acc_data);
	if (ret < 0) {
	...
	stat->accel_cali[2] = GRAVITY_HAL_UNIT - acc_data[2];

	dev_dbg(&stat->client->dev, "lsm6d calibrate data, x:%d, y:%d, z:%d\n",
...
static int lsm6d_gyr_cdev_set_poll_delay(
		struct sensors_classdev *sensors_cdev, unsigned int msecs)
{
	struct lsm6dx0_status *stat =
...
static int lsm6d_gyr_cdev_set_enable(struct sensors_classdev *sensors_cdev,
							unsigned int enable)
{
	struct lsm6dx0_status *stat =
		container_of(sensors_cdev, struct lsm6dx0_status, gyr_cdev);
...
static int32_t lsm6dx0_acc_get_data(struct lsm6dx0_status *stat, int32_t *xyz)
{
	...
	for (i = 0; i < 3; i++) {
		xyz[i] = (int32_t)stat->pdata_main->rot_matrix[0][i] * hw_d[0] +
				(int32_t)stat->pdata_main->rot_matrix[1][i] * hw_d[1] +
				(int32_t)stat->pdata_main->rot_matrix[2][i] * hw_d[2];
		xyz[i] = (xyz[i] * GRAVITY_HAL_UNIT) / GRAVITY_REG_UNIT;
	}
...
static int32_t lsm6dx0_gyr_get_data(struct lsm6dx0_status *stat, int32_t *xyz)
{
	...
	hw_d[0] = (int16_t)(gyro_data[1] << 8 | gyro_data[0]);
	hw_d[1] = (int16_t)(gyro_data[3] << 8 | gyro_data[2]);
	hw_d[2] = (int16_t)(gyro_data[5] << 8 | gyro_data[4]);
		...
		const struct i2c_device_id *id)
{
	...
	stat->acc_cdev = sensors_accel_cdev;
	stat->acc_cdev.sensors_enable = lsm6d_acc_cdev_set_enable;
	stat->acc_cdev.sensors_poll_delay = lsm6d_acc_cdev_set_poll_delay;
