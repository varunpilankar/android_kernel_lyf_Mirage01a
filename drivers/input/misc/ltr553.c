...
static uint16_t lux_level_spirit[LUX_LEVEL] = {0, 10, 50, 100, 200, 1000};
...
static void report_ps_input_event(struct ltr553_data *ltr553)
{
	...
	if (adc_value > ltr553->default_ps_highthresh) {
		...
		input_event(ltr553->ps_input_dev, EV_SYN, SYN_TIME_SEC,
				ktime_to_timespec(timestamp).tv_sec);
		input_event(ltr553->ps_input_dev, EV_SYN, SYN_TIME_NSEC,
...
static void report_als_input_event(struct ltr553_data *ltr553)
{
	...
	input_event(ltr553->als_input_dev, EV_SYN, SYN_TIME_SEC,
				ktime_to_timespec(timestamp).tv_sec);
	input_event(ltr553->als_input_dev, EV_SYN, SYN_TIME_NSEC,
				ktime_to_timespec(timestamp).tv_nsec);
	input_sync(ltr553->als_input_dev);
...
static int8_t als_enable_init(struct ltr553_data *ltr553)
{
	...
	"edge jump" with FW. default:0x03, bug 99519. */
				...
				struct ltr553_platform_data *ltr_pdata)
{
	...
	if (rc) {
		...
		else if(!strncmp(project_name, "spirit", strlen("spirit")))
		{
			spirit_flage = 1;
			lux_level = lux_level_spirit;
		}
		else
			lux_level = lux_level_default;
	}
