...
unsigned long swch_irq_start = 0;	//add for ignore btn when headset is inserting
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	swch_irq_start = jiffies;	//add for ignore btn when headset is inserting
...
//add for ignore btn when headset is inserting
	unsigned long msec = 0;
	unsigned long interval;

	interval = jiffies - swch_irq_start;
	msec = jiffies_to_msecs(interval);
	...
	pr_debug("~~~ljq ...interval= %ld, msec=%ld ,swch_irq_start= %ld \n",interval,msec,swch_irq_start);
        if(msec<400)
	{
		pr_err("btn press ignore because of msec=%ld",msec);
		goto done;
	}
	result1 = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN_RESULT);
